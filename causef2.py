...
df_path = './tfm/data/ccm_dataset'
var_names = ['air_temperature_2m', 'gross_primary_productivity', 'latent_energy', 'precipitation', 'surface_moisture']
data_format = 'nc'
substract_seasonality = [True, True, True, True, True, True]
cycle_length = 46

data = DataLoader(df_path = df_path, var_names = var_names, data_format = data_format, substract_seasonality = substract_seasonality, cycle_length=cycle_length)
...
link_assumptions[5] = {
    (i, -tau): '<?-' if (i, -tau) != (5, 0) and i != 5 else 'o?o'
    for i in range(N)
    for tau in range(tau_max + 1)
    if (i, -tau) != (5, 0)  # Filtering condition to exclude (1, 0)
}

link_assumptions[0] = {
    (i, -tau): '-?>' if (i, -tau) != (0, 0) and i == 5 else 'o?o'
    for i in range(N)
    for tau in range(tau_max + 1)
...
link_assumptions[1] = {
    (i, -tau): '-?>' if (i, -tau) != (1, 0) and i == 5 else 'o?o'
    for i in range(N)
    for tau in range(tau_max + 1)
...
link_assumptions[2] = {
    (i, -tau): '-?>' if (i, -tau) != (2, 0) and i == 5 else 'o?o'
    for i in range(N)
    for tau in range(tau_max + 1)
...
link_assumptions[3] = {
    (i, -tau): '-?>' if (i, -tau) != (3, 0) and i == 5 else 'o?o'
    for i in range(N)
    for tau in range(tau_max + 1)
...
link_assumptions[4] = {
    (i, -tau): '-?>' if (i, -tau) != (4, 0) and i == 5 else 'o?o'
    for i in range(N)
    for tau in range(tau_max + 1)
    if (i, -tau) != (4, 0)  # Filtering condition to exclude (1, 0)
}
